...
import XMonad.Actions.CycleWS (Direction1D(..), moveTo, shiftTo, WSType(..), nextScreen, prevScreen, nextWS, prevWS, shiftToNext, shiftToPrev, toggleWS)
import XMonad.Layout.Spacing
import XMonad.Hooks.DynamicLog (def, dynamicLogWithPP, wrap, xmobarPP, xmobarColor, shorten, PP(..))
...
--
myWorkspaces    = ["default","code","music","terminal","vbox","6","7","8","9"]

-- Border colors for unfocused and focused windows, respectively.
--
myNormalBorderColor  = "#5b828f"
myFocusedBorderColor = "#b3ebff"
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm, xK_i     ), spawn "subl")
    , ((modm, xK_g     ), spawn "nautilus")

    , ((modm, xK_o     ), spawn "discord")
    ...
    , ((mod1Mask, xK_k     ), windows W.swapDown  )

    -- Swap the focused window with the previous window
    , ((mod1Mask, xK_j     ), windows W.swapUp    )
    ...
    , ((mod1Mask .|. shiftMask, xK_j), decWindowSpacing 4)
    , ((mod1Mask .|. shiftMask, xK_k), incWindowSpacing 4)


    , ((mod1Mask .|. shiftMask, xK_Up), toggleWS)

    -- , ((mod1Mask .|. shiftMask, xK_Right),        -- a crazy keybinding!
    --     do t <- findWorkspace getSortByXineramaRule Next (Not emptyWS) 2
    --     windows . view $ t                                         )

    -- , ((mod1Mask .|. shiftMask, xK_Left),        -- a crazy keybinding!
    --     do t <- findWorkspace getSortByXineramaRule Prev (Not emptyWS) 2
    --     windows . view $ t                                         )
...
--        | (key, sc) <- zip [xK_Right, xK_Left, xK_r] [0..]
...
type ScreenFoo = W.Screen WorkspaceId (Layout Window) Window ScreenId ScreenDetail

visibleScreens :: WindowSet -> [ScreenFoo]
visibleScreens cws = ([W.current cws]) ++ (W.visible cws)
...
myLogHookForPipe currentWindowSet (i, xmobarPipe) =
    ...
    joinToString $ map (W.tag . W.workspace) $  -- extract workspace names and join into a single string
    filter ((==) (S i) . W.screen) $  -- filter to this xmobar's screen only
...
main = do
    ...
    -- xmproc0 <- spawnPipe "xmobar -x 0 /home/jsv/.config/xmobar/xmobarrc"
    -- xmproc1 <- spawnPipe "xmobar -x 1 /home/jsv/.config/xmobar/xmobarrc"
    n <- countScreens
